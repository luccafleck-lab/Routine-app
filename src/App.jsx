...
const T={bg:"#F7F7F5",surface:"#FFFFFF",border:"#E4E4E0",border2:"#EFEFEC",text:"#0F0F0F",text2:"#6B6B6B",text3:"#ABABAB",accent:"#2563EB",accentL:"#EEF3FF",accentM:"#BFCFFD",red:"#DC2626",redL:"#FEF2F2",orange:"#EA580C",orangeL:"#FFF7ED",amber:"#D97706",amberL:"#FFFBEB",green:"#16A34A",greenL:"#F0FDF4",purple:"#7C3AED",purpleL:"#F5F3FF",teal:"#0D9488",tealL:"#F0FDFA"};
const COMPANIES=[{id:"el-distrito",name:"El Distrito",icon:"🏕️"},{id:"distrito-facas",name:"Distrito Facas",icon:"🔪"},{id:"pablo-severo",name:"Pablo Severo",icon:"🚗"},{id:"pessoal",name:"Pessoal",icon:"👤"},{id:"outros",name:"Outros",icon:"📁"}];
const URGENCY=[{id:"critico",label:"Crítico",sub:"Hoje",color:T.red,bg:T.redL,rank:0},{id:"urgente",label:"Urgente",sub:"2–3 dias",color:T.orange,bg:T.orangeL,rank:1},{id:"importante",label:"Importante",sub:"Esta semana",color:T.amber,bg:T.amberL,rank:2},{id:"normal",label:"Normal",sub:"Sem prazo",color:T.green,bg:T.greenL,rank:3},{id:"paralelo",label:"Paralelo",sub:"Quando sobrar",color:T.accent,bg:T.accentL,rank:4}];
const ENTRY_TYPES=[{id:"tarefa",label:"Tarefa",icon:"▫"},{id:"habito",label:"Hábito",icon:"◎"},{id:"pessoal",label:"Pessoal",icon:"◇"}];
const EVENT_CATS=[{id:"reuniao",label:"Reunião",icon:"⊙",color:T.accent},{id:"mentoria",label:"Mentoria",icon:"◉",color:T.purple},{id:"medico",label:"Médico",icon:"♦",color:T.red},{id:"pessoal",label:"Pessoal",icon:"◇",color:T.teal},{id:"outro",label:"Outro",icon:"○",color:T.text2}];
...
async function loadTasks(){const{data,error}=await supabase.from("tasks").select("*").eq("user_id","lucca").order("created_at",{ascending:true});if(error){console.error(error);return[];}return data;}
async function saveTask(task){const{data,error}=await supabase.from("tasks").upsert({...task,user_id:"lucca"}).select();if(error)console.error(error);return data;}
async function deleteTaskDB(id){await supabase.from("tasks").delete().eq("id",id);}
async function loadEvents(){const{data,error}=await supabase.from("events").select("*").eq("user_id","lucca").order("date",{ascending:true});if(error){console.error(error);return[];}return data;}
async function saveEvent(event){const{data,error}=await supabase.from("events").upsert({...event,user_id:"lucca"}).select();if(error)console.error(error);return data;}
async function deleteEventDB(id){await supabase.from("events").delete().eq("id",id);}
...
return(<div style={{background:T.surface,border:`1px solid ${expanded?T.accentM:T.border}`,borderRadius:12,marginBottom:6,overflow:"hidden",opacity:task.done?.4:1}}>
...
{expanded&&(<div style={{borderTop:`1px solid ${T.border2}`,padding:"10px 12px"}}>
...
return(<div style={{background:T.surface,border:`1px solid ${expanded?cat.color+"60":T.border}`,borderRadius:14,marginBottom:8,overflow:"hidden"}}>
...
{daysUntil!==null&&<div style={{fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:20,background:daysUntil===0?T.redL:daysUntil<=2?T.orangeL:T.border2,color:daysUntil===0?T.red:daysUntil<=2?T.orange:T.text3}}>{daysUntil===0?"Hoje":daysUntil===1?"Amanhã":`em ${daysUntil}d`}</div>}
</div>
{expanded&&(<div style={{borderTop:`1px solid ${T.border2}`,padding:"10px 14px"}}>
...
setStep("result");
}catch(e){setError("Erro ao gerar o plano.");setStep("form");}
};
const apply=()=>{
if(!plan)return;
const updates={};
...
return(<div style={{position:"fixed",inset:0,background:"#0008",zIndex:100,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={onClose}>
...
<div><div style={{fontSize:18,fontWeight:800,color:T.text}}>Planejar com IA</div>
<div style={{fontSize:12,color:T.text3,marginTop:2}}>Distribui as tarefas até o prazo</div></div>
<button onClick={onClose} style={{fontSize:20,background:"none",border:"none",color:T.text3,cursor:"pointer"}}>×</button>
</div>
{step==="form"&&(<>
<div style={{marginBottom:16}}>
<div style={{fontSize:11,fontWeight:700,color:T.text3,letterSpacing:".06em",marginBottom:8}}>PRAZO — EVENTO FIXO</div>
{upcomingEvs.length>0?upcomingEvs.map(e=>{const cat=evCat(e.cat);const sel=form.eventId===String(e.id);
return(<div key={e.id} onClick={()=>setForm(f=>({...f,eventId:sel?"":String(e.id),deadline:""}))} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:12,marginBottom:6,cursor:"pointer",border:`1px solid ${sel?cat.color+"80":T.border}`,background:sel?cat.color+"0D":T.bg}}>
<span style={{fontSize:18}}>{cat.icon}</span>
<div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:T.text}}>{e.title}</div>
<div style={{fontSize:11,color:T.text3}}>{fmtDate(e.date)}{e.time?` · ${e.time}`:""}</div></div>
...
<div style={{marginBottom:16}}>
<div style={{fontSize:11,fontWeight:700,color:T.text3,letterSpacing:".06em",marginBottom:8}}>TAREFAS</div>
{pendingTasks.map(t=>{const sel=form.taskIds.includes(t.id);const u=ugInfo(t.urgency);
return(<div key={t.id} onClick={()=>setForm(f=>({...f,taskIds:sel?f.taskIds.filter(id=>id!==t.id):[...f.taskIds,t.id]}))} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 12px",borderRadius:10,marginBottom:5,cursor:"pointer",border:`1px solid ${sel?T.accentM:T.border2}`,background:sel?T.accentL:T.bg}}>
<div style={{width:8,height:8,borderRadius:"50%",background:u.color,flexShrink:0}}/>
<div style={{flex:1,fontSize:12,color:T.text}}>{t.title}</div>
<div style={{fontSize:10,color:T.text3}}>{t.duration||60}min</div>
{sel&&<span style={{fontSize:13,color:T.accent}}>✓</span>}
</div>);})}
</div>
<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20}}>
<div><div style={{fontSize:11,fontWeight:700,color:T.text3,marginBottom:6}}>HORAS/DIA</div>
<div style={{display:"flex",gap:5}}>{[3,4,5,6].map(h=>(<button key={h} onClick={()=>setForm(f=>({...f,hoursPerDay:h}))} style={{flex:1,padding:"7px 0",borderRadius:8,fontSize:12,fontWeight:700,background:form.hoursPerDay===h?T.accent:T.bg,color:form.hoursPerDay===h?"#fff":T.text3,border:`1px solid ${form.hoursPerDay===h?T.accent:T.border}`,cursor:"pointer"}}>{h}h</button>))}</div></div>
<div><div style={{fontSize:11,fontWeight:700,color:T.text3,marginBottom:6}}>INÍCIO</div>
<div style={{display:"flex",gap:5}}>{[6,7,8,9].map(h=>(<button key={h} onClick={()=>setForm(f=>({...f,startHour:h}))} style={{flex:1,padding:"7px 0",borderRadius:8,fontSize:12,fontWeight:700,background:form.startHour===h?T.accent:T.bg,color:form.startHour===h?"#fff":T.text3,border:`1px solid ${form.startHour===h?T.accent:T.border}`,cursor:"pointer"}}>{h}h</button>))}</div></div>
</div>
...
<div style={{fontSize:14,fontWeight:600,color:T.text}}>Gerando seu plano…</div>
</div>)}
{step==="result"&&plan&&(<>
...
{plan.days.map(day=>(<div key={day.date} style={{marginBottom:14}}>
<div style={{fontSize:11,fontWeight:700,color:T.text3,marginBottom:8}}>{day.label||fmtDate(day.date)}</div>
{day.blocks.map((bl,i)=>(<div key={i} style={{display:"flex",gap:10,alignItems:"center",padding:"8px 12px",background:T.bg,border:`1px solid ${T.border}`,borderRadius:10,marginBottom:5}}>
<div style={{fontSize:11,color:T.accent,fontWeight:700,width:38,flexShrink:0}}>{bl.time}</div>
<div style={{flex:1,fontSize:12,color:T.text}}>{bl.title}</div>
...
<button onClick={apply} style={{flex:2,padding:"12px",borderRadius:12,fontSize:13,fontWeight:700,background:T.accent,color:"#fff",border:"none",cursor:"pointer"}}>✓ Aplicar</button>
</div>
</>)}
</div>
</div>);}
...
useEffect(()=>{(async()=>{const[t,e]=await Promise.all([loadTasks(),loadEvents()]);setTasks(t??[]);setEvents(e??[]);setReady(true);})();},[]);
const toggleTask=async id=>{const task=tasks.find(t=>t.id===id);if(!task)return;const updated={...task,done:!task.done};setTasks(ts=>ts.map(t=>t.id===id?updated:t));await saveTask(updated);};
const deleteTask=async id=>{setTasks(ts=>ts.filter(t=>t.id!==id));await deleteTaskDB(id);setExpanded(null);};
const deleteEvent=async id=>{setEvents(es=>es.filter(e=>e.id!==id));await deleteEventDB(id);setExpanded(null);};
const toggleExp=id=>setExpanded(e=>e===id?null:id);
const addTask=async()=>{if(!newTask.title.trim())return;setSaving(true);const saved=await saveTask(newTask);if(saved&&saved[0])setTasks(ts=>[...ts,saved[0]]);setNewTask({title:"",type:"tarefa",company:"el-distrito",urgency:"normal",notes:"",date:todayStr(),time:"",duration:60});setShowAdd(false);setSaving(false);};
const addEvent=async()=>{if(!newEvent.title.trim()||!newEvent.date)return;setSaving(true);const saved=await saveEvent(newEvent);if(saved&&saved[0])setEvents(es=>[...es,saved[0]]);setNewEvent({title:"",cat:"reuniao",date:todayStr(),time:"",duration:60,notes:"",company:"el-distrito"});setShowAdd(false);setSaving(false);};
const applyPlan=async updates=>{const updated=tasks.map(t=>updates[t.id]?{...t,...updates[t.id]}:t);setTasks(updated);await Promise.all(Object.keys(updates).map(id=>{const t=updated.find(t=>String(t.id)===String(id));return t?saveTask(t):null;}));};
...
<style>{`@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=Lora:wght@600;700&display=swap');*{box-sizing:border-box;margin:0;padding:0;}button{font-family:inherit;}input,textarea,select{font-family:inherit;}input:focus,textarea:focus,select:focus{outline:none;}::-webkit-scrollbar{width:0;}@keyframes fadeUp{from{opacity:0;transform:translateY(10px);}to{opacity:1;transform:none;}}.fu{animation:fadeUp .22s ease both;}@keyframes blink{0%,100%{opacity:1}50%{opacity:.3}}.blink{animation:blink 1.5s ease infinite;}@keyframes shimmer{0%{opacity:.4}50%{opacity:.8}100%{opacity:.4}}.shim{animation:shimmer 1.8s ease infinite;}`}</style>
...
<button key={t.id} onClick={()=>setView(t.id)} style={{padding:"8px 14px",fontSize:12,fontWeight:600,whiteSpace:"nowrap",color:view===t.id?T.accent:T.text3,background:"none",border:"none",borderBottom:view===t.id?`2px solid ${T.accent}`:"2px solid transparent",cursor:"pointer"}}>{t.label}</button>
))}
</div>
</header>
...
{todayEvents.length>0&&(<div style={{padding:"0 14px 14px"}}>
...
const isCur=sh===nowH();
return(<div key={slot} style={{display:"flex",gap:8,marginBottom:2,alignItems:"flex-start",borderRadius:8,padding:"3px 4px",background:isCur?T.accentL:"transparent",borderLeft:isCur?`3px solid ${T.accent}`:"3px solid transparent"}}>
...
{slotEvs.map(e=>{const cat=evCat(e.cat);return(<div key={e.id} style={{background:cat.color+"15",border:`1px solid ${cat.color}30`,borderRadius:8,padding:"5px 9px",marginBottom:3,cursor:"pointer"}} onClick={()=>toggleExp(e.id)}><span style={{fontSize:11,fontWeight:600,color:cat.color}}>{cat.icon} {e.title}</span></div>);})}
{slotTs.map(t=>{const u=ugInfo(t.urgency);return(<div key={t.id} style={{background:u.bg,border:`1px solid ${u.color}30`,borderRadius:8,padding:"5px 9px",marginBottom:3,cursor:"pointer",opacity:t.done?.4:1}} onClick={()=>toggleExp(t.id)}>
<span style={{fontSize:11,color:T.text}}>{t.title}</span>
{expanded===t.id&&<div style={{marginTop:6,display:"flex",gap:6}}>
<button onClick={e=>{e.stopPropagation();toggleTask(t.id);}} style={{padding:"4px 10px",borderRadius:7,fontSize:11,background:T.bg,border:`1px solid ${T.border}`,cursor:"pointer",color:T.text2}}>{t.done?"↩":"✓"}</button>
<button onClick={e=>{e.stopPropagation();deleteTask(t.id);}} style={{padding:"4px 10px",borderRadius:7,fontSize:11,background:T.redL,border:`1px solid ${T.red}30`,color:T.red,cursor:"pointer"}}>Excluir</button>
</div>}
</div>);})}
...
return(<div key={d} style={{background:T.surface,border:`1px solid ${isT?T.accent:T.border}`,borderRadius:14,marginBottom:8,overflow:"hidden"}}>
...
if(item._ev){const cat=evCat(item.cat);return(<div key={"e"+item.id} style={{display:"flex",gap:8,alignItems:"center",padding:"5px 0",borderBottom:`1px solid ${T.border2}`}}><span>{cat.icon}</span><div style={{flex:1,fontSize:12,fontWeight:600,color:cat.color}}>{item.title}</div>{item.time&&<span style={{fontSize:10,color:T.text3}}>{item.time}</span>}</div>);}
const u=ugInfo(item.urgency);
return(<div key={"t"+item.id} style={{display:"flex",gap:8,alignItems:"center",padding:"5px 0",borderBottom:`1px solid ${T.border2}`,opacity:item.done?.4:1}}><div style={{width:6,height:6,borderRadius:"50%",background:u.color,flexShrink:0}}/><div style={{flex:1,fontSize:12,color:T.text,textDecoration:item.done?"line-through":"none"}}>{item.title}</div>{item.time&&<span style={{fontSize:10,color:T.text3}}>{item.time}</span>}</div>);
})}
</div>)}
...
const dT=dayTasks(d),dE=dayEvents(d),isT=d===todayStr();
return(<div key={d} onClick={()=>{setSelDate(d);setView("today");}} style={{background:isT?T.accent:T.surface,border:`1px solid ${isT?T.accent:T.border}`,borderRadius:9,padding:"5px 3px",textAlign:"center",cursor:"pointer",minHeight:48,display:"flex",flexDirection:"column",alignItems:"center"}}>
...
{tasks.filter(t=>t.done).length>0&&(<div style={{marginTop:8}}>
...
<button onClick={()=>{setAddMode("task");setShowAdd(true);}} style={{width:52,height:52,borderRadius:"50%",background:T.accent,color:"#fff",fontSize:24,border:"none",boxShadow:`0 4px 20px ${T.accent}55`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
</div>)}
  {showAdd&&(
...
{["task","event"].map(m=>(<button key={m} onClick={()=>setAddMode(m)} style={{flex:1,padding:"7px",borderRadius:8,fontSize:13,fontWeight:700,background:addMode===m?T.surface:"transparent",color:addMode===m?T.text:T.text3,border:addMode===m?`1px solid ${T.border}`:"none",cursor:"pointer"}}>{m==="task"?"Tarefa / Hábito":"Evento fixo"}</button>))}
</div>
{addMode==="task"&&(<>
<div style={{marginBottom:12}}><div style={{fontSize:11,fontWeight:700,color:T.text3,marginBottom:6}}>TÍTULO *</div>
<input value={newTask.title} onChange={e=>setNewTask(n=>({...n,title:e.target.value}))} placeholder="Descreva a tarefa…" style={{width:"100%",padding:"10px 12px",borderRadius:10,border:`1px solid ${T.border}`,fontSize:14,color:T.text,background:T.bg}}/></div>
<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
<div><div style={{fontSize:11,fontWeight:700,color:T.text3,marginBottom:6}}>DATA</div>
<input type="date" value={newTask.date} onChange={e=>setNewTask(n=>({...n,date:e.target.value}))} style={{width:"100%",padding:"9px 10px",borderRadius:9,border:`1px solid ${T.border}`,fontSize:13,color:T.text,background:T.bg}}/></div>
<div><div style={{fontSize:11,fontWeight:700,color:T.text3,marginBottom:6}}>HORÁRIO</div>
<input type="time" value={newTask.time} onChange={e=>setNewTask(n=>({...n,time:e.target.value}))} style={{width:"100%",padding:"9px 10px",borderRadius:9,border:`1px solid ${T.border}`,fontSize:13,color:T.text,background:T.bg}}/></div>
</div>
<div style={{marginBottom:12}}><div style={{fontSize:11,fontWeight:700,color:T.text3,marginBottom:8}}>URGÊNCIA</div>
<div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{URGENCY.map(u=>(<button key={u.id} onClick={()=>setNewTask(n=>({...n,urgency:u.id}))} style={{padding:"5px 12px",borderRadius:20,fontSize:11,fontWeight:600,background:newTask.urgency===u.id?u.bg:"transparent",color:newTask.urgency===u.id?u.color:T.text3,border:`1px solid ${newTask.urgency===u.id?u.color+"60":T.border2}`,cursor:"pointer"}}>{u.label}</button>))}</div></div>
<div style={{marginBottom:12}}><div style={{fontSize:11,fontWeight:700,color:T.text3,marginBottom:8}}>EMPRESA</div>
<div style={{display:"flex",flexWrap:"wrap",gap:6}}>{COMPANIES.map(c=>(<button key={c.id} onClick={()=>setNewTask(n=>({...n,company:c.id}))} style={{padding:"5px 11px",borderRadius:20,fontSize:11,fontWeight:600,background:newTask.company===c.id?T.accentL:"transparent",color:newTask.company===c.id?T.accent:T.text3,border:`1px solid ${newTask.company===c.id?T.accentM:T.border2}`,cursor:"pointer"}}>{c.icon} {c.name}</button>))}</div></div>
<div style={{marginBottom:20}}><div style={{fontSize:11,fontWeight:700,color:T.text3,marginBottom:6}}>NOTAS</div>
<textarea value={newTask.notes} onChange={e=>setNewTask(n=>({...n,notes:e.target.value}))} placeholder="Contexto adicional…" rows={2} style={{width:"100%",padding:"10px 12px",borderRadius:10,border:`1px solid ${T.border}`,fontSize:13,color:T.text,background:T.bg,resize:"none"}}/></div>
...
{addMode==="event"&&(<>
<div style={{marginBottom:12}}><div style={{fontSize:11,fontWeight:700,color:T.text3,marginBottom:6}}>CATEGORIA</div>
<div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{EVENT_CATS.map(cat=>(<button key={cat.id} onClick={()=>setNewEvent(n=>({...n,cat:cat.id}))} style={{padding:"6px 13px",borderRadius:20,fontSize:12,fontWeight:600,background:newEvent.cat===cat.id?cat.color+"18":"transparent",color:newEvent.cat===cat.id?cat.color:T.text3,border:`1px solid ${newEvent.cat===cat.id?cat.color+"60":T.border2}`,cursor:"pointer"}}>{cat.icon} {cat.label}</button>))}</div></div>
<div style={{marginBottom:12}}><div style={{fontSize:11,fontWeight:700,color:T.text3,marginBottom:6}}>TÍTULO *</div>
<input value={newEvent.title} onChange={e=>setNewEvent(n=>({...n,title:e.target.value}))} placeholder="Ex: Mentoria tráfego pago…" style={{width:"100%",padding:"10px 12px",borderRadius:10,border:`1px solid ${T.border}`,fontSize:14,color:T.text,background:T.bg}}/></div>
<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
<div><div style={{fontSize:11,fontWeight:700,color:T.text3,marginBottom:6}}>DATA *</div>
<input type="date" value={newEvent.date} onChange={e=>setNewEvent(n=>({...n,date:e.target.value}))} style={{width:"100%",padding:"9px 10px",borderRadius:9,border:`1px solid ${T.border}`,fontSize:13,color:T.text,background:T.bg}}/></div>
<div><div style={{fontSize:11,fontWeight:700,color:T.text3,marginBottom:6}}>HORÁRIO</div>
<input type="time" value={newEvent.time} onChange={e=>setNewEvent(n=>({...n,time:e.target.value}))} style={{width:"100%",padding:"9px 10px",borderRadius:9,border:`1px solid ${T.border}`,fontSize:13,color:T.text,background:T.bg}}/></div>
</div>
<div style={{marginBottom:12}}><div style={{fontSize:11,fontWeight:700,color:T.text3,marginBottom:6}}>DURAÇÃO</div>
<div style={{display:"flex",gap:6}}>{[30,60,90,120,180].map(d=>(<button key={d} onClick={()=>setNewEvent(n=>({...n,duration:d}))} style={{flex:1,padding:"7px 0",borderRadius:8,fontSize:11,fontWeight:700,background:newEvent.duration===d?T.accentL:"transparent",color:newEvent.duration===d?T.accent:T.text3,border:`1px solid ${newEvent.duration===d?T.accentM:T.border}`,cursor:"pointer"}}>{d<60?`${d}m`:d===60?"1h":`${d/60}h`}</button>))}</div></div>
<div style={{marginBottom:20}}><div style={{fontSize:11,fontWeight:700,color:T.text3,marginBottom:6}}>NOTAS</div>
<textarea value={newEvent.notes} onChange={e=>setNewEvent(n=>({...n,notes:e.target.value}))} placeholder="Observações…" rows={2} style={{width:"100%",padding:"10px 12px",borderRadius:10,border:`1px solid ${T.border}`,fontSize:13,color:T.text,background:T.bg,resize:"none"}}/></div>
<div style={{display:"flex",gap:10}}>
<button onClick={()=>setShowAdd(false)} style={{flex:1,padding:"11px",borderRadius:12,fontSize:13,fontWeight:600,background:T.bg,border:`1px solid ${T.border}`,color:T.text2,cursor:"pointer"}}>Cancelar</button>
